...
def translate_text(text, target_language):
    try:
        ...
        if "quota" in error_message.lower() or "exceeded" in error_message.lower():
            return st.error("Translation failed due to API quota exhaustion. Please wait or upgrade your plan to continue.")
...
st.set_page_config(page_title="English Translator and Text-to-Speech App", page_icon="🌐", layout="centered")
...
st.markdown(
    "<h1 style='font-size:30px;'>🌍 Google Gemini English Translator & Text-to-Speech</h1>",
    unsafe_allow_html=True
)

# App description
st.markdown(
    "<p style='color:#E65C4F; font-style:italic; font-weight:bold; text-align:center;'>Effortlessly translate from English into your chosen language and hear it spoken aloud.</p>",
    unsafe_allow_html=True
)
...
if st.button("Translate"):
    ...
    if not user_input.strip():
        ...
        try:
            ...
            lang_code_to_name = {val:key for key, val in language.items()}
            full_lang_name = lang_code_to_name.get(detected_lang, detected_lang)
            st.info(f"Detected Input Language: {full_lang_name}")
        except Exception as e:
            st.error(f"Unable to detect input language: {e}")
            st.stop()
